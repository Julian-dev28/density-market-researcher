// ============================================================
// Self-contained pipeline for the web dashboard.
//
// Inlines fixture data + transform logic from src/ so that
// web/ is a standalone Next.js app with no cross-directory
// imports — required for Vercel deployment.
// ============================================================
...
// ---------------------------------------------------------------------------
// Registries (from fredRegistry.ts, cryptoRegistry.ts, sectors.ts)
// ---------------------------------------------------------------------------
...
>([
  ["DFF",          { seriesId: "DFF",          name: "Federal Funds Effective Rate",             category: "INTEREST_RATES", inverseSentiment: true  }],
  ["DGS10",        { seriesId: "DGS10",        name: "10-Year Treasury Constant Maturity Rate",  category: "INTEREST_RATES", inverseSentiment: true  }],
  ["T10Y2Y",       { seriesId: "T10Y2Y",       name: "10-Year minus 2-Year Treasury Spread",     category: "INTEREST_RATES", inverseSentiment: false }],
  ["CPIAUCSL",     { seriesId: "CPIAUCSL",     name: "Consumer Price Index (All Urban)",         category: "INFLATION",      inverseSentiment: true  }],
  ["CPILFESL",     { seriesId: "CPILFESL",     name: "Core CPI (ex Food & Energy)",              category: "INFLATION",      inverseSentiment: true  }],
  ["PPIACO",       { seriesId: "PPIACO",       name: "Producer Price Index (All Commodities)",   category: "INFLATION",      inverseSentiment: true  }],
  ["UNRATE",       { seriesId: "UNRATE",       name: "Unemployment Rate",                        category: "EMPLOYMENT",     inverseSentiment: true  }],
  ["PAYEMS",       { seriesId: "PAYEMS",       name: "Total Nonfarm Payrolls",                   category: "EMPLOYMENT",     inverseSentiment: false }],
  ["GDP",          { seriesId: "GDP",          name: "Gross Domestic Product",                   category: "GROWTH",         inverseSentiment: false }],
  ["INDPRO",       { seriesId: "INDPRO",       name: "Industrial Production Index",              category: "GROWTH",         inverseSentiment: false }],
  ["BAMLH0A0HYM2", { seriesId: "BAMLH0A0HYM2", name: "ICE BofA US High Yield OAS",             category: "CREDIT",         inverseSentiment: true  }],
  ["MORTGAGE30US", { seriesId: "MORTGAGE30US", name: "30-Year Fixed Mortgage Rate",              category: "HOUSING",        inverseSentiment: true  }],
  ["HOUST",        { seriesId: "HOUST",        name: "Housing Starts",                           category: "HOUSING",        inverseSentiment: false }],
  ["UMCSENT",      { seriesId: "UMCSENT",      name: "University of Michigan Consumer Sentiment",category: "SENTIMENT",      inverseSentiment: false }],
]);
...
const METRICS_BY_ID = new Map<
  string,
  { metricId: string; name: string; category: CryptoMetricCategory; unit: string; source: string; inverseSentiment: boolean }
>([
  ["TOTAL_MARKET_CAP",     { metricId: "TOTAL_MARKET_CAP",     name: "Total Crypto Market Cap",     category: "MARKET_STRUCTURE", unit: "USD",          source: "CMC_GLOBAL", inverseSentiment: false }],
  ["BTC_DOMINANCE",        { metricId: "BTC_DOMINANCE",        name: "Bitcoin Dominance",            category: "MARKET_STRUCTURE", unit: "%",            source: "CMC_GLOBAL", inverseSentiment: true  }],
  ["ETH_DOMINANCE",        { metricId: "ETH_DOMINANCE",        name: "Ethereum Dominance",           category: "MARKET_STRUCTURE", unit: "%",            source: "CMC_GLOBAL", inverseSentiment: false }],
  ["TOTAL_VOLUME_24H",     { metricId: "TOTAL_VOLUME_24H",     name: "Total 24h Trading Volume",     category: "LIQUIDITY",        unit: "USD",          source: "CMC_GLOBAL", inverseSentiment: false }],
  ["STABLECOIN_MARKET_CAP",{ metricId: "STABLECOIN_MARKET_CAP",name: "Stablecoin Market Cap",        category: "STABLECOINS",      unit: "USD",          source: "CMC_GLOBAL", inverseSentiment: false }],
  ["DEFI_MARKET_CAP",      { metricId: "DEFI_MARKET_CAP",      name: "DeFi Token Market Cap",        category: "DEFI",             unit: "USD",          source: "CMC_GLOBAL", inverseSentiment: false }],
  ["DEFI_TVL",             { metricId: "DEFI_TVL",             name: "DeFi Total Value Locked",      category: "DEFI",             unit: "USD",          source: "DEFILLAMA",  inverseSentiment: false }],
  ["FEAR_GREED",           { metricId: "FEAR_GREED",           name: "Crypto Fear & Greed Index",    category: "SENTIMENT",        unit: "index (0-100)",source: "FEARGREED",  inverseSentiment: true  }],
]);

// ---------------------------------------------------------------------------
// Fixture data (from ingestion/*fixture* functions)
// ---------------------------------------------------------------------------
...
function mkObs(date: string, value: string): FREDObs & { realtime_start: string; realtime_end: string } {
...
interface CMCData { btc_dominance: number; eth_dominance: number; quote: { USD: { total_market_cap: number; total_volume_24h: number; altcoin_market_cap: number; defi_market_cap: number; defi_volume_24h: number; defi_24h_percentage_change: number; stablecoin_market_cap: number; stablecoin_volume_24h: number; stablecoin_24h_percentage_change: number; total_market_cap_yesterday_percentage_change: number; last_updated: string } } }
interface CMC { status: { timestamp: string; error_code: number }; data: CMCData }
...
function deriveMacroSignal(
  ...
  if (def.inverseSentiment) {
    ...
    if (rising) return { signal: "BEARISH", rationale: `${def.name} trending higher — headwind for equities.` };
  ...
  if (rising) return { signal: "BULLISH", rationale: `${def.name} trending higher.` };
...
function inferRegime(indicators: MacroIndicator[]): Regime {
  ...
  if (unrate) score += (unrate.periodDelta ?? 0) > 0 ? -2 : 1;
  if (dff) score += (dff.periodDelta ?? 0) < 0 ? 1 : -1;
  if (spread) score += spread.latestValue < 0 ? -1 : 1;
  if (sentiment) score += sentiment.latestValue < 70 ? -1 : 1;
...
function sectorSignal(
  ...
  const bullishCount = (dayChangePct > 0 ? 1 : 0) + ((monthChangePct ?? 0) > 0 ? 1 : 0) + ((rsVsSPY ?? 0) > 0 ? 1 : 0);
...
function deriveCryptoSignal(
  ...
  switch (metricId) {
    ...
    case "STABLECOIN_MARKET_CAP":
      ...
      if (rising) return { signal: "BULLISH", rationale: "Stablecoin supply growing — dry powder accumulating." };
    ...
    case "DEFI_TVL":
      ...
      return { signal: "BEARISH", rationale: "DeFi TVL declining — capital leaving DeFi." };
    default: {
      if (rising === null) return { signal: "NEUTRAL", rationale: "Insufficient data." };
      const bullishDir = def.inverseSentiment ? !rising : rising;
      return { signal: bullishDir ? "BULLISH" : "BEARISH", rationale: `${def.name} ${rising ? "rising" : "declining"}.` };
    }
  }
}
...
function inferCryptoRegime(metrics: CryptoMetric[]): CryptoRegime {
  ...
  if (fg) { if (fg.latestValue >= 65) score += 2; else if (fg.latestValue >= 50) score += 1; else if (fg.latestValue <= 25) score -= 2; else score -= 1; }
  if (btcDom) { if (btcDom.latestValue < 45) score += 2; else if (btcDom.latestValue < 52) score += 1; else if (btcDom.latestValue > 62) score -= 1; }
  if (totalMC) { const mc = totalMC.periodDeltaPct ?? 0; if (mc > 5) score += 2; else if (mc > 0) score += 1; else if (mc < -10) score -= 2; else score -= 1; }
  if (tvl) { const tc = tvl.periodDeltaPct ?? 0; if (tc > 5) score += 1; else if (tc < -10) score -= 1; }
...
function categorySignal(
  ...
  switch (slug) {
    case "bitcoin": {
      ...
      if (bear && highDom) return { signal: "BULLISH", rationale: "Risk-off rotation into BTC. Dominance rising." };
      if (bull) return { signal: "BULLISH", rationale: "Bull market — BTC leading." };
      return { signal: "NEUTRAL", rationale: "Mixed signals for BTC." };
    }
    case "ethereum": {
      const tvlBull = (tvl?.signal ?? "NEUTRAL") === "BULLISH";
      if (bull && tvlBull) return { signal: "BULLISH", rationale: "Bull market with growing TVL — ETH ecosystem strong." };
      if (bear) return { signal: "BEARISH", rationale: "Risk-off pressuring ETH." };
      return { signal: "NEUTRAL", rationale: "ETH ecosystem neutral." };
    }
    case "defi": {
      const tvlSig = tvl?.signal ?? "NEUTRAL";
      if (tvlSig === "BULLISH" && bull) return { signal: "BULLISH", rationale: "TVL growing in bull market — DeFi constructive." };
      if (tvlSig === "BEARISH" || bear) return { signal: "BEARISH", rationale: "TVL declining or risk-off — DeFi under pressure." };
      return { signal: "NEUTRAL", rationale: "DeFi neutral." };
    }
    case "stablecoins":
      return (stables?.periodDeltaPct ?? 0) > 0
        ? { signal: "BULLISH", rationale: "Stablecoin supply growing — dry powder building." }
        : { signal: "NEUTRAL", rationale: "Stablecoin supply stable." };
    case "altcoins": {
      const fgVal = fg?.latestValue ?? 50;
      const lowDom = (btcDom?.latestValue ?? 55) < 48;
      if (regime === "ALT_SEASON" || (lowDom && fgVal > 55)) return { signal: "BULLISH", rationale: "Alt season — BTC dominance falling, sentiment positive." };
      if (bear || fgVal < 30) return { signal: "BEARISH", rationale: "Risk-off — alts underperform in downturns." };
...
function transformCryptoMetrics(cmc: CMC, currentTvl: number, priorTvl: number | null, fgValue: number, fgPrior: number | null): CryptoMetric[] {
  ...
  const raw = [
    ...
    { metricId: "BTC_DOMINANCE",          value: cmc.data.btc_dominance,    prior: null },
    { metricId: "ETH_DOMINANCE",          value: cmc.data.eth_dominance,    prior: null },
    { metricId: "TOTAL_VOLUME_24H",       value: usd.total_volume_24h,      prior: null },
  ...
  return raw.map(({ metricId, value, prior }) => {
    ...
    return {
      metricId, name: def.name, category: def.category, source: def.source, unit: def.unit,
      latestValue: fmt(value), latestDate: today,
      priorValue: prior !== null ? fmt(prior) : null,
      periodDelta: delta, periodDeltaPct: deltaPct,
      signal, signalRationale: rationale, lastUpdated: now,
    };
  });
}
...
function transformCryptoCategories(metrics: CryptoMetric[], cmc: CMC): CategorySnapshot[] {
  ...
  const today = new Date().toISOString().split("T")[0];
  const now = new Date().toISOString();
  const dayChange = fmt(usd.total_market_cap_yesterday_percentage_change);
  ...
  return cats.map(({ slug, name, marketCap, dominance, drivers }) => {
    const { signal, rationale } = categorySignal(slug, metrics, regime);
    return {
      snapshotId: `${slug}_${today}`, categoryName: name, categorySlug: slug,
      totalMarketCapUsd: fmt(marketCap), dayChangePct: dayChange,
      dominancePct: fmt(dominance), cryptoRegime: regime,
      primaryMetricDrivers: drivers, categorySignal: signal, signalRationale: rationale, ingestedAt: now,
    };
  });
}
...
export function runPipeline(): PipelineOutput {
  const indicators: MacroIndicator[] = fredFixture()
    .map(transformFREDResult)
    .filter((i): i is MacroIndicator => i !== null);

  const sectors = transformSectorData(sectorFixture(), indicators);

  const cmc = cmcFixture();
  const cryptoMetrics = transformCryptoMetrics(cmc, 88_500_000_000, 95_200_000_000, 44, 38);
  const categories = transformCryptoCategories(cryptoMetrics, cmc);
